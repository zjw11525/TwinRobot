...
Array Fkine_Final(Theta theta)
{
	...
	//¼ÆËãÈý½Çº¯Êý
	double s1 = sin_(Theta1);
	double c1 = cos_(Theta1);

	double s2 = sin_(Theta2);
	double c2 = cos_(Theta2);

	double Theta23 = Theta2 + Theta3;

	double s23 = sin_(Theta23);
	double c23 = cos_(Theta23);

	double s4 = sin_(Theta4);
	double c4 = cos_(Theta4);

	double s5 = sin_(Theta5);
	double c5 = cos_(Theta5);

	double s6 = sin_(Theta6);
	double c6 = cos_(Theta6);
...
Theta Ikine_Step(Array T, Theta Angle_Last)
{
	...
	for (int r1 = 1; r1 < 3; r1++) // loop1
	{
		...
		{
			if (r2 == 1)
				angle2 = asin_(C / (sqrt_(pow_(A, 2) + pow_(B, 2)))) - D;
			else
				angle2 = PI - asin_(C / (sqrt_(pow_(A, 2) + pow_(B, 2)))) - D;

			c2 = cos_(angle2);
			s23 = sin_(angle2 + angle3);
			angle1 = atan2_((py - d6 * ay)*(a2*c2 - d4 * s23), (px - d6 * ax)*(a2*c2 - d4 * s23));
			for (int r3 = 1; r3 < 3; r3++) // loop3
			{
				c1 = cos_(angle1);
				s1 = sin_(angle1);
				c23 = cos_(angle2 + angle3);
				...
				{
					angle4 = atan2_(-ay36 * s5, -ax36 * s5);
					angle6 = atan2_(-oz36 * s5, nz36*s5);
				}
